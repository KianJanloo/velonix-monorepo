...
function heightMm(t: CompType): number {
  switch (t) {
    ...
    case "hex": return 3;
    case "bag": return 8;
    case "standee": return 25;
    case "spinner": return 4;
    case "track": return 3;
    case "sand_timer": return 20;
    case "line": return 1;
    default: return 6;
  }
}
...
function stdMat(
  pool: ResourcePool,
  color: THREE.Color,
  opts: { roughness?: number; metalness?: number; map?: THREE.Texture; emissiveIntensity?: number } = {},
...
function buildCapsuleGroup(
  ...
  const cyl = new THREE.Mesh(
    pool.geometry(`cap-cyl:${q(radius)}:${q(length)}`, () => new THREE.CylinderGeometry(radius, radius, length, 12)),
    mat,
  );
  cyl.castShadow = true;
  g.add(cyl);
  for (const y of [length / 2, -length / 2]) {
    const half = new THREE.Mesh(
      pool.geometry(`cap-hemi:${q(radius)}`, () => new THREE.SphereGeometry(radius, 12, 8, 0, Math.PI * 2, 0, Math.PI / 2)),
      mat,
    );
    half.position.y = y;
...
function buildDisc(pool: ResourcePool, diam: number, h: number, color: THREE.Color, metal: boolean): THREE.Object3D {
  ...
  const body = new THREE.Mesh(cyl(48), mat); body.position.y = h / 2; body.castShadow = true; body.receiveShadow = true;
  const rimMat = stdMat(pool, new THREE.Color(0xf5c451), { metalness: 0.8, roughness: 0.2, emissiveIntensity: 0.3 });
  const ring = new THREE.Mesh(
    pool.geometry(`rim:${q(r)}`, () => new THREE.TorusGeometry(r * 0.92, Math.max(0.004, diam * 0.03), 8, 32)),
    rimMat,
  );
  ring.rotation.x = Math.PI / 2; ring.position.y = h;
  near.add(body, ring);

  const mid = new THREE.Mesh(cyl(20), mat); mid.position.y = h / 2; mid.castShadow = true;
  const far = new THREE.Mesh(cyl(8), mat); far.position.y = h / 2;
  lod.addLevel(near, 0); lod.addLevel(mid, 6); lod.addLevel(far, 9);
  return lod;
}
...
function buildCard(pool: ResourcePool, w: number, d: number, h: number, color: THREE.Color, map?: THREE.Texture): THREE.Object3D {
  ...
  lod.addLevel(near, 0); lod.addLevel(far, 7);
  return lod;
}
 ...
 */
function buildDie(pool: ResourcePool, size: number, color: THREE.Color, pipColor: THREE.Color): THREE.Object3D {
  const g = new THREE.Group();
  const half = size / 2;

  const mat = stdMat(pool, color, { roughness: 0.3, metalness: 0.15 });
  const cube = new THREE.Mesh(
    pool.geometry(`cube:${q(size)}`, () => new THREE.BoxGeometry(size, size, size)),
    mat,
  );
  cube.position.y = half; cube.castShadow = true; cube.receiveShadow = true;
  g.add(cube);
    ...
    for (const a of [-1, 1]) {
      for (const b of [-1, 1]) {
        const edge = new THREE.Mesh(
          pool.geometry(`bevel:${q(edgeR)}:${q(edgeL)}`, () => new THREE.CylinderGeometry(edgeR, edgeR, edgeL, 8)),
          bevelMat,
        );
        if (axis === "x") { edge.rotation.z = Math.PI / 2; edge.position.set(0, half + a * (half - edgeR), b * (half - edgeR)); }
        if (axis === "y") { edge.position.set(a * (half - edgeR), half, b * (half - edgeR)); }
        if (axis === "z") { edge.rotation.x = Math.PI / 2; edge.position.set(a * (half - edgeR), half + b * (half - edgeR), 0); }
        g.add(edge);
      }
    }
  }

  const pipMat = stdMat(pool, pipColor, { roughness: 0.5, metalness: 0.0, emissiveIntensity: 0.2 });
  const pipGeo = pool.geometry(`pip:${q(size)}`, () => new THREE.SphereGeometry(size * 0.07, 8, 6));
  const o = size * 0.28;
  const pip = (px: number, py: number, pz: number) => {
    const m = new THREE.Mesh(pipGeo, pipMat); m.position.set(px, py + half, pz); g.add(m);
  };

  // Face 1 (+Y) — 1 pip
  pip(0, size * 0.51, 0);
  // Face 6 (−Y) — 6 pips  
  for (const [fx, fz] of [[-o,-o],[o,-o],[-o,0],[o,0],[-o,o],[o,o]]) pip(fx!, -size * 0.51, fz!);
  // Face 2 (−Z front) — 2 pips
  pip(-o, o * 0.4, -size * 0.51); pip(o, -o * 0.4, -size * 0.51);
  // Face 5 (+Z back) — 5 pips
  pip(-o, o, size * 0.51); pip(o, o, size * 0.51); pip(0, 0, size * 0.51); pip(-o, -o, size * 0.51); pip(o, -o, size * 0.51);
  // Face 3 (+X right) — 3 pips
  pip(size * 0.51, o, -o); pip(size * 0.51, 0, 0); pip(size * 0.51, -o, o);
  // Face 4 (−X left) — 4 pips
  pip(-size * 0.51, o, -o); pip(-size * 0.51, o, o); pip(-size * 0.51, -o, -o); pip(-size * 0.51, -o, o);
...
function buildPawn(pool: ResourcePool, diam: number, height: number, color: THREE.Color): THREE.Object3D {
  const g = new THREE.Group();
  const mat = stdMat(pool, color, { roughness: 0.28, metalness: 0.18, emissiveIntensity: 0.1 });

  // Base — wide flat disc
  const base = new THREE.Mesh(
    pool.geometry(`pBase:${q(diam)}:${q(height)}`, () => new THREE.CylinderGeometry(diam * 0.44, diam * 0.5, height * 0.14, 32)),
    mat,
  );
  base.position.y = height * 0.07; base.castShadow = true; base.receiveShadow = true;
  g.add(base);

  // Shaft — tapered cylinder
  const shaft = new THREE.Mesh(
    pool.geometry(`pShaft:${q(diam)}:${q(height)}`, () => new THREE.CylinderGeometry(diam * 0.16, diam * 0.36, height * 0.52, 20)),
    mat,
  );
  shaft.position.y = height * 0.41; shaft.castShadow = true;
  g.add(shaft);

  // Collar — small disc at neck
  const collar = new THREE.Mesh(
    pool.geometry(`pCol:${q(diam)}:${q(height)}`, () => new THREE.CylinderGeometry(diam * 0.22, diam * 0.18, height * 0.06, 20)),
    mat,
  );
  collar.position.y = height * 0.7; collar.castShadow = true;
  g.add(collar);

  // Head — sphere (r128-safe)
  const head = new THREE.Mesh(
    pool.geometry(`pHead:${q(diam)}`, () => new THREE.SphereGeometry(diam * 0.25, 20, 16)),
    mat,
  );
...
function buildMeeple(pool: ResourcePool, w: number, height: number, color: THREE.Color): THREE.Object3D {
  ...
  // Head
  const head = new THREE.Mesh(
    pool.geometry(`mH:${q(u)}`, () => new THREE.SphereGeometry(u * 0.2, 16, 12)),
    mat,
  );
  head.position.y = height * 0.82; head.castShadow = true; g.add(head);

  // Torso — cylinder
  const torso = new THREE.Mesh(
    pool.geometry(`mT:${q(u)}:${q(height)}`, () => new THREE.CylinderGeometry(u * 0.16, u * 0.26, height * 0.42, 14)),
    mat,
  );
  torso.position.y = height * 0.5; torso.castShadow = true; g.add(torso);

  // Arms — capsule substitute
  const armR = buildCapsuleGroup(pool, mat, u * 0.07, height * 0.28);
  armR.position.set(u * 0.28, height * 0.54, 0); armR.rotation.z = Math.PI / 4;
  const armL = buildCapsuleGroup(pool, mat, u * 0.07, height * 0.28);
  armL.position.set(-u * 0.28, height * 0.54, 0); armL.rotation.z = -Math.PI / 4;
  g.add(armR, armL);
...
function buildBoard(pool: ResourcePool, w: number, d: number, color: THREE.Color, map?: THREE.Texture): THREE.Object3D {
  const g = new THREE.Group();
  ...
  const slab = new THREE.Mesh(
    pool.geometry(`board:${q(w)}:${q(d)}`, () => new THREE.BoxGeometry(w, thickness, d)),
    stdMat(pool, new THREE.Color(0x2a1f12), { roughness: 0.95 }),
  );
  slab.position.y = thickness / 2; slab.receiveShadow = true; slab.castShadow = true;
  g.add(slab);

  // Top felt surface (slightly raised)
  const felt = new THREE.Mesh(
    pool.geometry(`felt:${q(w)}:${q(d)}`, () => new THREE.BoxGeometry(w - frameW * 2, 0.008, d - frameW * 2)),
    stdMat(pool, color, { roughness: 0.95, metalness: 0, ...(map ? { map } : {}) }),
  );
  felt.position.y = thickness + 0.004; felt.receiveShadow = true;
  g.add(felt);

  // Gold border line
  const edgeLine = new THREE.LineSegments(
    pool.geometry(`boardEdge:${q(w)}:${q(d)}`, () => new THREE.EdgesGeometry(new THREE.BoxGeometry(w, thickness + 0.012, d))),
    pool.material("edge:gold", () => new THREE.LineBasicMaterial({ color: 0xf5c451, transparent: true, opacity: 0.4 })),
  );
...
function buildStandee(pool: ResourcePool, w: number, h: number, color: THREE.Color): THREE.Object3D {
  const g = new THREE.Group();
  ...
  const body = new THREE.Mesh(
    pool.geometry(`standee:${q(w)}:${q(h)}:${q(thickness)}`, () => new THREE.BoxGeometry(w, h * 0.75, thickness)),
    mat,
  );
  body.position.y = h * 0.75 / 2 + h * 0.12; body.castShadow = true;
  g.add(body);

  // Base tab
  const base = new THREE.Mesh(
    pool.geometry(`standeeBase:${q(w)}`, () => new THREE.BoxGeometry(w, h * 0.12, thickness * 2.5)),
    mat,
  );
  base.position.y = h * 0.06; base.castShadow = true; base.receiveShadow = true;
  g.add(base);
...
function buildBag(pool: ResourcePool, diam: number, h: number, color: THREE.Color, innerColor: THREE.Color): THREE.Object3D {
  console.log(h);
  const g = new THREE.Group();
  const r = diam * 0.42;
  ...
  body.scale.y = 1.15;
  body.position.y = r * 1.1; body.castShadow = true;
  g.add(body);

  // Neck / cinch
  const neck = new THREE.Mesh(
    pool.geometry(`bagNeck:${q(r)}`, () => new THREE.CylinderGeometry(r * 0.22, r * 0.38, r * 0.4, 16)),
    stdMat(pool, innerColor, { roughness: 0.7 }),
  );
  neck.position.y = r * 2.1; neck.castShadow = true;
  g.add(neck);

  // Cord knot (tiny sphere)
  const knot = new THREE.Mesh(
    pool.geometry(`bagKnot:${q(r)}`, () => new THREE.SphereGeometry(r * 0.12, 8, 6)),
...
function buildSpinner(pool: ResourcePool, diam: number, h: number, color: THREE.Color, needleColor: THREE.Color): THREE.Object3D {
  console.log(color);
  const g = new THREE.Group();
  const r = diam / 2;
  ...
  const needle = new THREE.Mesh(
    pool.geometry(`needle:${q(r)}`, () => new THREE.ConeGeometry(r * 0.06, r * 0.75, 8)),
    stdMat(pool, needleColor, { roughness: 0.3, metalness: 0.4, emissiveIntensity: 0.3 }),
  );
...
function buildSandTimer(pool: ResourcePool, diam: number, h: number, color: THREE.Color): THREE.Object3D {
  const g = new THREE.Group();
  const r = diam / 2;
  const halfH = h / 2;
  const mat = stdMat(pool, color, { roughness: 0.15, metalness: 0.05, emissiveIntensity: 0.08 });
  const sandMat = stdMat(pool, new THREE.Color(0xe8d5b8), { roughness: 0.9 });

  // Top bulb
  const top = new THREE.Mesh(
    pool.geometry(`stTop:${q(r)}:${q(halfH)}`, () => new THREE.ConeGeometry(r, halfH * 0.85, 20, 1, true)),
    mat,
  );
  top.position.y = h * 0.75; top.rotation.x = Math.PI; top.castShadow = true;
  g.add(top);

  // Bottom bulb
  const bot = new THREE.Mesh(
    pool.geometry(`stBot:${q(r)}:${q(halfH)}`, () => new THREE.ConeGeometry(r, halfH * 0.85, 20, 1, true)),
    mat,
  );
  bot.position.y = h * 0.25; bot.castShadow = true;
  g.add(bot);

  // Waist disc
  const waist = new THREE.Mesh(
    pool.geometry(`stWaist:${q(r)}`, () => new THREE.CylinderGeometry(r * 0.08, r * 0.08, h * 0.06, 12)),
  ...
  const sand = new THREE.Mesh(
    pool.geometry(`stSand:${q(r)}`, () => new THREE.ConeGeometry(r * 0.7, halfH * 0.4, 16)),
    sandMat,
  );
  ...
  for (const y of [0, h]) {
    const cap = new THREE.Mesh(
      pool.geometry(`stCap:${q(r)}`, () => new THREE.CylinderGeometry(r * 1.05, r * 1.05, h * 0.05, 20)),
      stdMat(pool, new THREE.Color(0xd4a030), { metalness: 0.7, roughness: 0.2 }),
    );
    cap.position.y = y;
    g.add(cap);
  }
...
function buildTrack(pool: ResourcePool, w: number, d: number, h: number, color: THREE.Color, accentColor: THREE.Color, spaces = 10): THREE.Object3D {
  const g = new THREE.Group();
  const mat = stdMat(pool, color, { roughness: 0.7 });

  const slab = new THREE.Mesh(
    pool.geometry(`track:${q(w)}:${q(d)}:${q(h)}`, () => new THREE.BoxGeometry(w, h, d)),
    mat,
  );
  slab.position.y = h / 2; slab.castShadow = true; slab.receiveShadow = true;
  g.add(slab);

  // Dividers
  const divMat = stdMat(pool, accentColor, { roughness: 0.5, metalness: 0.1, emissiveIntensity: 0.2 });
  const divH = h * 1.8;
  for (let i = 1; i < spaces; i++) {
    const x = -w / 2 + (i / spaces) * w;
    const div = new THREE.Mesh(
      pool.geometry(`trackDiv:${q(d)}:${q(divH)}`, () => new THREE.BoxGeometry(0.008, divH, d)),
      divMat,
    );
...
export function buildBoardScene(
  ...
  scene.add(boardBase); added.push(boardBase);
  ...
  comps.filter((c) => c.visible).forEach((c, i) => {
    ...
    switch (c.type) {
      ...
      case "die":
      case "cube":
        obj = buildDie(pool, diam, color, pipColor);
        break;
      case "pawn":
        obj = buildPawn(pool, diam, h, color);
        break;
      case "meeple":
        obj = buildMeeple(pool, w, h, color);
        break;
      case "token":
      case "coin":
      case "marker":
    ...
    if ((c.type === "text" || c.type === "note") && c.text) {
      ...
      const plane = new THREE.Mesh(
        pool.geometry(`plane:${q(w)}:${q(d)}`, () => new THREE.PlaneGeometry(w, d)),
        pmat,
      );
